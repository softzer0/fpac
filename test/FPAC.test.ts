...
import { FPAC, OracleHub, PegEngine } from "../typechain-types/contracts";
...
describe("FPAC Token", function () {
  ...
  describe("Minting", function () {
    it("Should allow authorized minter to mint tokens", async function () {
      ...
      await pegEngine.connect(owner).manualIntervention("mint", mintAmount, "Test mint");
    ...
    it("Should not allow unauthorized minting", async function () {
      ...
      await expect(fpac.connect(user1).mint(user1.address, mintAmount)).to.be.reverted;
    });
    ...
    it("Should not exceed max supply", async function () {
      ...
      await expect(pegEngine.connect(owner).manualIntervention("mint", excessiveAmount, "Test excessive mint")).to.be.reverted;
    });
  });
  ...
  describe("Emergency Functions", function () {
    it("Should allow admin to emergency mint", async function () {
      ...
      await fpac.connect(owner).emergencyMint(user1.address, mintAmount, "Emergency mint test");
    ...
    it("Should allow admin to pause contract", async function () {
      ...
      await expect(fpac.connect(user1).transfer(user2.address, 100)).to.be.reverted;
    });
  ...
  describe("Parameter Updates", function () {
    ...
    it("Should not allow invalid peg parameters", async function () {
      await expect(fpac.connect(owner).updatePegParameters(0, 100)).to.be.reverted; // Invalid target price

      await expect(fpac.connect(owner).updatePegParameters(INITIAL_PRICE, 1001)).to.be.reverted; // Tolerance too high
    });
  });
  ...
  describe("Access Control", function () {
    it("Should only allow admin to grant roles", async function () {
      ...
      await expect(fpac.connect(user1).grantRole(MINTER_ROLE, user2.address)).to.be.reverted;
    });
