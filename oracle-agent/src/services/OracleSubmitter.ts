...
export class OracleSubmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Initialize contract
      this.oracleHub = new ethers.Contract(this.config.network.oracleHubAddress, ORACLE_HUB_ABI, this.wallet);
  ...
  async submitData(feedName: string, value: bigint | number, confidence: number): Promise<void> {
    try {
      ...
      // Estimate gas
      const gasEstimate = await this.oracleHub.submitData.estimateGas(feedName, valueBI, confidence);

      // Add 20% buffer to gas estimate
      const gasLimit = (gasEstimate * 120n) / 100n;

      // Submit transaction
      const tx = await this.oracleHub.submitData(feedName, valueBI, confidence, { gasLimit });
  ...
  private async shouldSubmitData(feedName: string, newValue: bigint, newConfidence: number): Promise<boolean> {
    try {
      ...
      const difference = newValue > currentValueBI ? newValue - currentValueBI : currentValueBI - newValue;
