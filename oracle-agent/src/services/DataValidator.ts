import { BasketData, CPIData, PriceData } from "./EconomicDataFetcher";
...
export class DataValidator {
  ...
  validateBasketData(data: BasketData): boolean {
    ...
    if (difference / data.totalValue > 0.001) {
      // 0.1% tolerance
      return false;
    }
  ...
  validatePriceData(data: PriceData): boolean {
    ...
    if (data.price <= 0 || data.price > 10) {
      // FAIT should be around $1, max $10
      return false;
    }
